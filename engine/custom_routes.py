...
async def analyze_custom(
    ...
    skip_memo: bool = Query(default=False, description="Quick toggle to skip AI memo (for testing)"),
    ...
    - skip_memo: Quick toggle (query param) - if true, skips AI memo regardless of include_ai_memo
    """
    # Quick toggle: skip_memo query param overrides include_ai_memo
    if skip_memo:
        include_ai_memo = False
    ...
    try:
        ...
        # Step 5: Compute validity using Bavella
        validity_data = None
        try:
            from .bavella_adapter import check_portfolio_validity
            
            # z_score might be a Series - get last value
            z_score_val = None
            if z_score is not None:
                if hasattr(z_score, 'iloc'):
                    z_score_val = float(z_score.iloc[-1])
                elif isinstance(z_score, (int, float)):
                    z_score_val = float(z_score)
            
            validity_output = check_portfolio_validity(
                portfolio_returns=daily_returns,
                regime_summary=regime_summary.to_dict() if hasattr(regime_summary, 'to_dict') else regime_summary,
                portfolio_name=analysis_name,
                owner_id="custom_upload",
                z_score_override=z_score_val,
            )
            validity_data = validity_output.to_dict()
            logger.info(f"Custom upload validity: {validity_output.score} ({validity_output.state})")
        except Exception as e:
            logger.warning(f"Bavella validity analysis failed: {e}")
            validity_data = None
    ...
    # For custom uploads, period is the actual data length
    analysis_period_days = len(daily_returns)
...
async def analyze_custom_pdf(
    ...
    context: str = Form(default=""),
    include_ai_memo: bool = Form(default=True),
    skip_memo: bool = Query(default=False, description="Quick toggle to skip AI memo"),
    x_openai_key: Optional[str] = Header(None, alias="X-OpenAI-Key"),
):
    """
...
def _add_confidentiality_badge(html: str, has_dates: bool, parsed: ParsedData) -> str:
    ...
    <div style="background: rgba(79, 195, 247, 0.1); 
                ...
                color: #4fc3f7;
                font-size: 13px;">
        <strong>ℹ</strong> Your data has been deleted.
    </div>
    '''
